...
users = User.all
users.each do |user|
  random_store_count = rand(1..20)
  store = Store.find(random_store_count)
  5.times do |order|
    t1 = Time.parse("2012-11-16 12:00:00")
    t2 = Time.parse("2018-09-15 12:00:00")
    created = Order.create(user_id: user.id, status: weighed_number({0 => 0.2, 1 => 0.1, 2 => 0.1, 3 => 0.6 }), created_at: rand(t1..t2), updated_at: Time.now, store_id: store.id)
    random = weighed_number({1 => 0.2, 2 => 0.2, 3 => 0.2, 4 => 0.2, 5 => 0.2 })
    random.times do |add_item|
      created.items << Item.where(store_id: store.id)
    end
    puts "Created order #{created.id} for #{created.user.name}"
  end
  5.times do |order|
